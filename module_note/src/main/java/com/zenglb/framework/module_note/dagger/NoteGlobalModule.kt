...
/**
 * Kotlin 中使用dagger :  https://www.jianshu.com/p/053a77c79075
 *
 *
 * 全局的东西
 *
 * Note MODULE  使用的全局的东西，API Service 等
 *
 * Created by anylife.zlb@gmail.com on 2018/1/11.
 */
@Module
class NoteGlobalModule {
    ...
    @Provides
    @Singleton
    fun provideNoteApiService(spDao: SPDao, mContext: Context): NoteApiService {
        //Retrofit 的create 真是精华所在啊！
        //retrofit.create(Api.class) 动态代理

        //Return 的api对象其实是一个动态代理对象，并不是一个真正的NewsApiService接口的implements产生的对象

        return HttpRetrofit.getRetrofit(spDao, mContext).create(NoteApiService::class.java)
    }
