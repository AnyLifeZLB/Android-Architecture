...
class MyWorker(private val context: Context, private val workerParams: WorkerParameters) :
    ...
    override fun doWork(): Result {
        ...
        val workTask = if (workName == null) null else DozeWakeUpManger.taskMap[workName]

        Log.e(DozeWakeUpManger.TAG, "Keep alive 「class WrapWorker」  -!- work ${workName}:${workTask}")
...
//        TestBackgroundWork(applicationContext).testDozeWork()
        ...
        workTask?.run {
            mTask?.let {
                it.run()
                // 继续开始下一次“扫描”
                if (mIntervalMillis > TaskImpl.INTERVAL_NEVER) {
                    scheduleTask(it, mIntervalMillis, mIntervalMillis)
                    Log.d(DozeWakeUpManger.TAG, "继续开始下一次“扫描 \n \n")
                }
            }
        }
